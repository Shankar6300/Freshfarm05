  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        setUserId(decoded.email || decoded.id);
      } catch (err) {
    ...
    // Listen for new messages from farmer or other customers
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Notify via socket
        if (socket) {
          socket.emit('chat_message', { orderId: parseInt(orderId), message: newMsg });
        }
      }
    } catch (error) {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          <div>
            <p style={{ margin: '0 0 5px 0', fontSize: '12px', color: '#666' }}>Recipient</p>
            <p style={{ margin: 0, fontWeight: '500' }}>{order.buyerName}</p>
          </div>
          <div>
            <p style={{ margin: '0 0 5px 0', fontSize: '12px', color: '#666' }}>Phone</p>
            <p style={{ margin: 0, fontWeight: '500' }}>{order.buyerPhoneNumber}</p>
          </div>
          <div>
            <p style={{ margin: '0 0 5px 0', fontSize: '12px', color: '#666' }}>Delivery Address</p>
            <p style={{ margin: 0, fontWeight: '500', fontSize: '14px' }}>{order.buyerLocation}</p>
          </div>
          <div>
            <p style={{ margin: '0 0 5px 0', fontSize: '12px', color: '#666' }}>Total Amount</p>
            <p style={{ margin: 0, fontWeight: '600', fontSize: '16px', color: '#4CAF50' }}>₹{order.totalPrice}</p>
          </div>
        </div>
      </div>
            ...
            messages.map((msg, index) => {
              ...
              return (
                <div key={index} style={{
                  ...
                  <div style={{
                    backgroundColor: isCustomer ? '#4CAF50' : '#e0e0e0',
