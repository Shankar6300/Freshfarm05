...
const DashboardBox = ({ title, value, increase, lastMonth, icon, t }) => {
  return (
    <div style={{
      borderRadius: '15px',
      padding: '20px',
      ...
      <p style={{ marginBottom: '10px', color: '#6c757d', fontSize: '9px', lineHeight: '1.5' }}>
        {increase != null && `+${increase}% ${t('fromLastMonth')}`}
        {lastMonth && <span>, {t('lastMonthLabel')}: {lastMonth}</span>}
      </p>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios.get('https://d2pskbh3g9o3pk.cloudfront.net/api/admintotaldata')
      .then(response => {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', marginTop: '20px' }}>
        ...
        {/* Dashboard content */}
        <div style={{ flex: '1' }}>
          <h2 style={{ color: '#4CAF50' }}>{t('dashboard')}</h2>
