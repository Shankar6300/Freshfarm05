...
const API_BASE_URL = process.env.REACT_APP_BACKEND_URL?.replace(/\/$/, '') || 'https://d2pskbh3g9o3pk.cloudfront.net'; // Define your API base URL
...
const ProductTable = () => {
  ...
  const filteredProducts = products.filter(product =>
    getLocalizedProductName(product, language).toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            {filteredProducts.map((product) => (
              <tr key={product.id}>
                <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                  <img
                    src={String(product.image || '').startsWith('http://') || String(product.image || '').startsWith('https://') ? product.image : `${API_BASE_URL}/${product.image}`}
