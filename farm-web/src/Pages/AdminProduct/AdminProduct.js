...
const AdminProduct = () => {
  ...
  const filteredProducts = products.filter(product =>
    getLocalizedProductName(product, language).toLowerCase().includes(searchInput.toLowerCase())
  );
          ...
          <table style={{ width: '1150px', borderCollapse: 'collapse', marginLeft: '260px', fontSize: '12px' }}>
            ...
            {filteredProducts.map((product, index) => (
              <tr key={product.id} style={{ backgroundColor: index % 2 === 0 ? '#fff' : 'inherit' }}>
                ...
                <td style={{ border: '1px solid #ddd', padding: '8px', borderRight: '1px solid #ddd', borderLeft: '1px solid #fff' }}>{getLocalizedProductName(product, language)}</td>
