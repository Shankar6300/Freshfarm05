...
const FarmerManagement = () => {
  ...
  const validateFarmer = (farmer) => {
    ...
    if (!farmer.fullName.match(/^[A-Z]/)) {
      errors.fullName = t('capitalNameValidation');
    }
    if (!farmer.email.includes('@')) {
      errors.email = t('emailAtValidation');
    }
    if (!farmer.phoneNumber.match(/^\d{10}$/)) {
      errors.phoneNumber = t('phoneValidation');
    }
    return errors;
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await axios.put(`http://localhost:8081/farmers/${editedFarmer.id}`, editedFarmer);
  ...
  const handleDeleteConfirmation = (id) => {
    setDeleteMessage(t('deleteFarmerConfirm'));
    setDeleteFarmerId(id);
  };

  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8081/farmers/${deleteFarmerId}`);
    ...
    <div>
      ...
      <div className="container">
        <div className="farmer-management">
          <h1 style={{marginBottom:'30px',marginTop:'0px', fontWeight:'400', fontSize:'24px', marginLeft:'85px'}}>{t('farmersManagement')}</h1>
          <div className="container" style={{ marginBottom: '20px', marginRight:'470px', paddingTop:'0px'}}>
            ...
            <input
              ...
              placeholder={t('searchFarmerName')}
            ...
            <tbody>
              {filteredFarmers.map(farmer => (
                <tr key={farmer.id}>
                  ...
                  <td>
                    <button style={{backgroundColor:'#4CAF50', color:' white', border:'none', padding: '8px 14px', fontSize:'12px', borderRadius:'8px'}} onClick={() => handleEdit(farmer.id)}>{t('edit')}</button>
                    <button style={{backgroundColor:'#e2e2e2c0', color:' rgb(57, 57, 57)', border:'none',fontSize:'12px', padding: '8px 14px', borderRadius:'8px', marginLeft:'14px'}} onClick={() => handleDeleteConfirmation(farmer.id)} >{t('delete')}</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {deleteMessage && (
            <div className="delete-dialog" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', zIndex: '9999', maxWidth: '300px', width: '100%' }}>
              ...
              <button onClick={handleDelete} style={{  padding: '8px 25px', fontSize: '14px', border: 'none', borderRadius: '8px', cursor: 'pointer', marginLeft:'50px' }}>{t('ok')}</button>
              <button onClick={handleCancelDelete} style={{ marginLeft:'30px', padding: '8px 16px', fontSize: '14px', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>{t('cancel')}</button>
            </div>
          )}
{editPopupVisible && (
